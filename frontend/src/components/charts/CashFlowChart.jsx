...
export default function CashFlowChart({ data, accent, onBarClick }) {
  ...
  const svgW = Math.max(w, padL + padR + 60 * groups);
  ...
  const getScrollLeft = () => outerRef.current ? outerRef.current.scrollLeft : 0;

  return (
    <div style={{ overflowX: 'auto', width: '100%' }} ref={outerRef}>
      <div className="chart-wrap" ref={ref}>
        {w > 0 && (
          <svg width={svgW} height={H} role="img">
            {ticks.map((t, i) => (
              <g key={i}>
                <line x1={padL} x2={svgW - padR} y1={yOf(t)} y2={yOf(t)} className="grid-line" strokeDasharray={i === 0 ? '0' : '3 5'} />
        ...
        {hover != null && data[hover] && (
          <div className="chart-tip" style={{
            left: xOf(hover) - getScrollLeft(),
            top: 70,
            transform: (xOf(hover) - getScrollLeft()) > w - 150 ? 'translate(-100%, -50%)' : 'translate(12px, -50%)'
          }}>
